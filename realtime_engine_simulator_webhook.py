...
# -*- coding: utf-8 -*-
"""
实时发动机RUL预测模拟系统 (Webhook版本)

模拟真实发动机运行，每10分钟滑动窗口向前移动一次，
循环遍历CMaps数据集进行实时RUL预测，并通过webhook发送预测结果。
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('simulation_webhook.log', encoding='utf-8'),
...
class RealtimeEngineSimulatorWebhook:
    """实时发动机模拟器 (Webhook版本)"""

    def __init__(self, engine_id=1, data_file="CMaps/test_FD001.txt",
                 rul_file="CMaps/RUL_FD001.txt", model_path=None,
                 webhook_url="http://54.238.253.106:5678/webhook-test/e72eda5c-1e4b-4466-8c0b-b3d385119a78"):
        """
        初始化模拟器
        ...
        # 文件路径
        self.results_file = "simulation_results_webhook.csv"
        self.state_file = "simulation_state_webhook.json"
        ...
        logger.info(f"初始化发动机模拟器 (Webhook版本) - 发动机ID: {engine_id}")
    ...
    def send_webhook(self, predicted_rul, processing_time, window_data):
        """发送预测结果到webhook"""
        try:
            # 构造webhook数据
            webhook_data = {
                ...
                "prediction": {
                    ...
                    "window_start_cycle": int(self.window_start_cycle),
                    "window_end_cycle": int(self.window_end_cycle),
                ...
                "engine_info": {
                    ...
                    "total_predictions": int(self.total_predictions + 1)
                },
                "model_info": {
                    "model_file": Path(self.predictor.model_path).name if hasattr(self.predictor, 'model_path') else 'unknown',
                    "prediction_count": int(self.total_predictions + 1)
                },
                "simulation_info": {
                    "simulation_start_time": self.simulation_start_time.isoformat() if self.simulation_start_time else None,
                    "webhook_success_count": int(self.webhook_success_count),
                    "webhook_failure_count": int(self.webhook_failure_count)
                }
            }
                ...
                for col in numeric_columns:
                    if col not in ['unit_number', 'time_in_cycles']:
                        sensor_stats[col] = {
                            ...
                            "std": float(window_data[col].std()) if len(window_data[col]) > 1 else 0.0
                        }
                webhook_data["sensor_statistics"] = sensor_stats

            # 发送HTTP POST请求
            headers = {
                'Content-Type': 'application/json',
                'User-Agent': f'EngineSimulator/{self.engine_id}'
            }
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Webhook发送成功 - 状态码: {response.status_code}")
            ...
            self.webhook_failure_count += 1
            logger.error(f"Webhook发送请求异常: {e}")
            return False
        except Exception as e:
            self.webhook_failure_count += 1
    ...
    def save_prediction_result(self, predicted_rul, processing_time):
        ...
        try:
            ...
            result = {
                ...
                'webhook_success': self.webhook_success_count,
                'webhook_failure': self.webhook_failure_count
            }
            ...
            with open(self.results_file, 'a', encoding='utf-8') as f:
                if not file_exists:
                    ...
                    f.write('timestamp,engine_id,window_start_cycle,window_end_cycle,'
                           'predicted_rul,model_version,processing_time,webhook_success,webhook_failure\n')
                ...
                f.write(f"{result['timestamp']},{result['engine_id']},"
                       ...
                       f"{result['processing_time']:.3f},{result['webhook_success']},{result['webhook_failure']}\n")
        ...
        try:
            if os.path.exists(self.state_file):
                ...
                if state.get('engine_id') == self.engine_id:
                    ...
                    self.webhook_success_count = state.get('webhook_success_count', 0)
                    self.webhook_failure_count = state.get('webhook_failure_count', 0)
                    ...
                    logger.info(f"恢复状态 - 位置: {self.current_position}, "
                               ...
                               f"Webhook成功: {self.webhook_success_count} 次, "
                               f"Webhook失败: {self.webhook_failure_count} 次")
                    return True
                else:
    ...
    def run_simulation_cycle(self):
        ...
        try:
            ...
            # 发送webhook
            webhook_success = self.send_webhook(predicted_rul, processing_time, window_data)
            ...
            logger.info(f"模拟周期完成 - 窗口: {self.window_start_cycle}-{self.window_end_cycle}, "
                       ...
                       f"Webhook: {'成功' if webhook_success else '失败'}")
    ...
    def start_simulation(self, cron_expression="*/10 * * * *"):
        ...
        try:
            logger.info("=" * 60)
            logger.info("启动实时发动机模拟系统 (Webhook版本)")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='实时发动机RUL预测模拟系统 (Webhook版本)')
    ...
    parser.add_argument('--webhook-url',
                       default='http://54.238.253.106:5678/webhook-test/e72eda5c-1e4b-4466-8c0b-b3d385119a78',
                       help='Webhook URL地址')
    ...
    try:
        # 创建模拟器
        simulator = RealtimeEngineSimulatorWebhook(
